...
# тестов (от 0 до 100).
# Также экземпляр должен сообщать средний балл по тестам для каждого и
...
class Validate:                                  # Дескриптор на проверку в ФИО наличия первой заглавной буквы,
...
class Student:
    ...
    @lessons.setter
    def lessons(self, lessons_file: Path):                      # Декоратор считывает с файла список предметов в словарь lessons
    ...
    def __call__(self, lesson: str, number: int, type_est: str = "lesson"):     # Метод для сохранения новой оценки по предмету/тесту
        ...
        if type_est == "lesson":
            ...
            self.lessons["lessons"][lesson]["test_results"].append(number)
                                                                               ...
                                                                               # Cредний бал по тестам по каждому предмету
    ...
    @staticmethod
    def calculate_middle_estimate(lessons: dict) -> float:                      # Подсчет средней оценки по всем предметам
    ...
    @staticmethod
    def calculate_middle_test(lessons: dict) -> float:                           # Подсчет среднего бала по тестам по всем предметам"
...
if __name__ == '__main__':
    student_1 = Student("Светлана", "Геннадьевна", "Ишкинина", Path('lessons.csv'))
    ...
    student_2 = Student("Кристина", "Олеговна", "Москвина", Path('lessons.csv'))
